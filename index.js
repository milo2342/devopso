...
function guildConfig(guildId) {
  ...
  const singularTicketHandlerKeys = ['leoHandlerRoleIds', 'liveryHandlerRoleIds', 'commissionHandlerRoleIds', 'qaTicketHandlerRoleIds'];
...
async function notifyQAOutcomeMembers(guild, submission, resultMessage = null) {
  ...
  for (const userId of recipients) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`Your ${submission.kind === 'vehicle' ? 'vehicle' : 'YMAP'} submission in **${guild.name}** has been reviewed.`)
      .addFields(
        { name: submission.kind === 'vehicle' ? 'Spawn Code' : 'Resource / Map', value: `\`${truncate(submission.identifier, 200)}\``, inline: true },
        { name: 'Status', value: submission.status, inline: true },
        { name: 'QA Reason', value: truncate(submission.reviewReason || 'No reason provided.'), inline: false },
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Mark a transferred vehicle as live in game and notify the original requester')
    .addStringOption((o) => o.setName('spawn_code').setDescription('Vehicle spawn code').setRequired(true))
...
async function handleDevConfig(interaction) {
  ...
  if (sub === 'add-role' || sub === 'remove-role') {
    ...
    const singularTicketHandlerTypes = new Set(['leo_handler', 'livery_handler', 'commission_handler', 'qa_ticket_handler']);
    if (sub === 'add-role') {
    ...
    const singularNote = ['leo_handler', 'livery_handler', 'commission_handler', 'qa_ticket_handler'].includes(type) && sub === 'add-role' ? ' Only this role will be pinged and granted handler access for that ticket type.' : '';
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === 'vehicle-in-game') {
        ...
        const asset = findAssets(interaction.guildId, spawnCode, 'vehicle')[0];
        if (!asset) return interaction.editReply(`No vehicle asset was found for spawn code **${spawnCode}**. Submit it through QA first so the bot can track the requester.`);
        if (asset.qaStatus !== 'Approved') return interaction.editReply(`Vehicle **${spawnCode}** is not QA approved yet (current status: ${asset.qaStatus || 'Unknown'}).`);
        ...
        if (transfer) {
          ...
          if (wasPending) {
            ...
            if (!alreadyBatched) {
              createdBatch = { id: shortId('batch_'), guildId: interaction.guildId, itemIds: [transfer.id], completedAt: now, completedBy: interaction.user.id, source: 'vehicle-in-game' };
        ...
        const resultEmbed = new EmbedBuilder()
          .setTitle('Vehicle Added In Game')
          .setDescription('This approved vehicle has been transferred into the server and is now available in game.')
          .addFields(
            { name: 'Spawn Code', value: `\`${truncate(asset.identifier, 200)}\``, inline: true },
            { name: 'Vehicle', value: truncate(asset.displayName || asset.identifier), inline: true },
        ...
        if (notifyChannelId) {
          ...
          if (notifyChannel?.isTextBased()) {
            await notifyChannel.send({
              content: requesterId ? `<@${requesterId}> your development item is now **in game**.` : undefined,
              embeds: [resultEmbed],
        ...
        if (requesterId) {
          ...
          if (requester) {
            dmDelivered = await requester.send({
              content: `Your vehicle **${asset.displayName || asset.identifier}** (\`${asset.identifier}\`) is now in game in **${interaction.guild.name}**.`,
              embeds: [resultEmbed],
        ...
        await sendConfiguredLog(interaction.guild, 'transfer', {
          embeds: [auditEmbed('Vehicle Added In Game', interaction.user, [
            { name: 'Spawn Code', value: `\`${truncate(asset.identifier, 200)}\``, inline: true },
        ...
        return interaction.editReply(`Marked **${asset.displayName || asset.identifier}** (\`${asset.identifier}\`) as in game.${ticketNotified ? ' The original ticket/source channel was notified.' : ''}${requesterId ? (dmDelivered ? ' The requester was also DMed.' : ' I could not DM the requester.') : ' No original requester was found.'}`);
      }
